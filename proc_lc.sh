...
security_groups=$(jq -r '.LaunchConfigurations[0].SecurityGroups | map("\"" + . + "\"") | join(",")' "./lc_faulty.json")
instance_type=$(echo "$faulty_json" | jq -r '.LaunchConfigurations[0].InstanceType')
